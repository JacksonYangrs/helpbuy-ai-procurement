...
> 版本：v1.1（由《设计方案_采购需求聚合子系统.md》「§6 AI 方案」章节独立抽出，并扩展为覆盖采购需求聚合与下单执行两大平行子系统的总体 AI 方案）
> 定位：代采平台**总体 AI 方案文档**，服务于其下两大平行子系统（采购需求聚合、下单执行）；集中定义模型选型、多模态处理、成本、容错、迭代、开源闭源、多模型路由等共用 AI 基础设施，并落到两大应用场景；独立可评审、可演进
> 说明：本文档编号保持 **§6.x**，便于另两文档交叉引用；§6.1–§6.12 为总体与场景一（采购需求聚合），§6.13 为场景二（下单执行）。
...
### 6.1 总览：总体 AI 方案与两大应用场景

**两大应用场景**
- **场景一 · 采购需求聚合（采集层）**：多模态需求 → 结构化提取 → 拆单 → 分类识别（详见 §6.3–§6.6、§6.8）。
- **场景二 · 下单执行（直接下单）**：已确认子单在第三方电商自动选品 / 填单 / 下单 / 付款中的 AI 视觉兜底（详见 §6.13）。

**共用 AI 基础设施（与子系统解耦、独立演进）**
- 模型选型与路由（§6.2）：按 operation 在 config 指定模型，效果 / 成本权衡
- 多模态处理（§6.3 输入归一化、§6.9 图片选型）：文本 / 文件 / 图片统一为结构化上下文
- 成本控制（§6.7）、容错降级（§6.8）、模型迭代与语料（§6.10）、开源 vs 闭源（§6.11）、多模型路由（§6.12）

```
[总体 AI 基础设施] 模型路由 §6.2 · 多模态 §6.3/§6.9 · 成本 §6.7 · 容错 §6.8 · 迭代 §6.10 · 开源闭源 §6.11 · 路由 §6.12
        │
        ├─ 场景一 采购需求聚合：多模态 → 结构化提取(§6.3) → 拆单(§6.4) → 分类识别(§6.5) → 置信度(§6.6)
        └─ 场景二 下单执行：    页面截图 → 视觉 LLM/computer-use(§6.13) → 元素操作（确定性 DOM 跑不通时兜底）
```
...
- **图片**：默认采用**路线 A（视觉 LLM）**——将 `ocr_backend` 实现为视觉多模态模型，一步完成识图与要素抽取（详见 §6.9）。独立 OCR（路线 B）仅作为视觉 LLM 失败 / 超成本 / 超长扫描件时的**可插拔降级后端**，本期不实现，亦不主动作为默认路径（避免双链路复杂度与 Tesseract 中文表格识别误差）。
...
- **输入上下文组装**：
  ...
  - 图片 → `ocr_text`（由视觉 LLM 后端〔路线 A〕产出的文本；字段名沿用 `ocr_text` 仅表示"图片转出的文本"，来源即 §6.9 的路线 A 后端）
  - 多形态共存 → 拼接为一段上下文，一次提取
- **输出 Schema（核心字段）**：
  ```json
  {
...
### 6.9 图片识别选型：视觉 LLM vs 独立 OCR 后端
...
2. **默认采用路线 A（视觉 LLM）**：将 `extract_image(path, ocr_backend)` 的 `ocr_backend` 实现为视觉 LLM 调用（两步法：图→文本→统一结构化提取，复用现��� `EXTRACTION_SCHEMA`）。离散截图、清单照片、单据等场景简单、准、成本可控。
...
### 6.11 开源 vs 闭源模型选型对比
AI 调用的模型可在**闭源托管**与**开源自托管**之间选择，二者在技��、成本、合规上差异明显。本节对比并给出本系统选型建议（2026-07-31 评审）。
...
### 6.12 多模型路由：国内外模型并存（2026-07-31 补充）
...
4. **与微调（§6.10）叠加**：无论是国内还是国外微调模型，均只以 `config.model.name` / `base_url` 引用；多模型并存时，路由层按上述策略选择最终模型 ID。

**结论**：技术上国内客户可用到国外模型（由路由决定）；但出于数据合规，**默认应让国内客户数据走国内模型，国外模型仅用于已合规/脱敏的场景**。这是一个**配置与合规策略问题，不是代码限制**——现有模型抽象已支持任意路由组合。

---

### 6.13 应用场景二：下单执行（直接下单）的 AI

下单执行子系统在第三方电商完成「选品 → 填单 → 下单 → 付款」，其混合架构为「确定性 DOM 层（Playwright 主力）+ AI 视觉层（兜底）+ 人工确认闸（合规底线）」。本节只讲其中的 AI 部分（其余见《设计方案_下单执行子系统.md》）。

**为什么需要 AI（视觉兜底）**
- 确定性 DOM 层能跑通时（稳定选择器、固定表单）**不调 AI**——控成本、控不确定性。
- 当页面动态、无稳定选择器、或遇到滑块 / 简单验证码类元素时，确定性层失效，调用 **视觉 LLM / computer-use** 识别页面元素并操作（呼应 §6.9 视觉 LLM 路线 A）。
- 即：在场景一（采集）视觉 LLM 是图片输入的**默认通道**；在场景二（执行）它是确定性 DOM 跑不通时的**兜底通道**——两者一致，均把视觉能力作为核心，不依赖独立 OCR。

**技术实现要点**
- `ai_vision.py`：封装视觉 LLM / computer-use 调用，输入页面截图，输出"元素坐标 + 操作意图"，交引擎执行。
- 模型路由复用 §6.2 / §6.12：视觉兜底可配独立强模型（如闭源多模态），与采集场景的提取模型解耦，按 `operation` 在 config 路由。
- 成本控制复用 §6.7：每次视觉兜底调用结束即写 `ai_api_cost(operation=vision_fallback)`，单笔订单 AI 成本可核算。
- 容错复用 §6.8：视觉调用失败 → 回退人工，不阻断其他步骤。

**AI 治理边界：人工确认闸**
- 首登授权、付款前确认、风控转人工——关键节点必须人工把关（详见执行子系统 §5.3）。
- 原则：AI 不无人监管地动钱；人工闸是合规底线，即便视觉层再强也不绕过。

---

> 本文档是代采平台**总体 AI 方案**，服务于其下两大平行子系统：《设计方案_采购需求聚合子系统.md》（采购需求聚合）与《设计方案_下单执行子系统.md》（下单执行）。
> 两文档中以下引用均指向本文档：采购需求聚合 §3 提取管线（图片路线 A 见 §6.9）、§4 拆单粒度（见 §6.4）、AI 字段识别逻辑（business_type/payer_type 见 §6.5）、成本控制（见 §6.7）、图片选型（见 §6.9）、模型迭代（见 §6.10）、开源闭源（见 §6.11）、多模型路由（见 §6.12）；下单执行 §5.2 AI 视觉层（见 §6.13 应用场景二）。
