# 代采平台 · 采购需求聚合子系统 — AI 方案设计文档（独立文档）

> 版本：v1.0（自《设计方案_采购需求聚合子系统.md》的「§6 AI 方案」章节独立抽出）
> 定位：代采平台下两大平行子系统之一「采购需求聚合子系统」的 AI 方案子文档（另一平行子系统为下单执行子系统）；与主设计文档配套；独立可评审、可演进
> 说明：本文档编号保持 **§6.x**，以便主文档交叉引用（主文档 §3 / §4 中"见 §6.x"即指本文档）。

---

## 6. AI / LLM 方案

本文档说明系统里 LLM 的角色、调用方式、Prompt 策略、模型选型与成本/容错/合规机制。AI 不是一次性黑盒，而是**管线中的三段式处理**：`多模态内容 → 结构化提取 → 拆单 → 分类识别`。
...
原始消息(多模态)
   │
   ▼ [① 内容提取 Extraction]
   │   文本直送 / 文件解析 / 图片(视觉LLM后端) → 文本 → 统一结构化要素（含 items[]、客户线索、金额、时间…，图片策略见 §6.9）
   ▼ [② 拆单 Splitting]
   │   items[] 按「客户+商品+时间」展开为 N 个候选子单，归入一个父单
   ▼ [③ 分类识别 Classification]
       结合 customer.contract_term 判定 business_type / payer_type
       产出 confidence，低置信子单置 status = AI待确认
```
每一步都是一次（或多次）LLM 调用，每次调用结束即写一行 `ai_api_cost`。
...
> 本文档配套主文档：《设计方案_采购需求聚合子系统.md》（系统架构 / 数据模型 / 采集调度）。
> 主文档中以下引用均指向本文档：§3 提取管线（图片路线 A 见 §6.9）、§4 拆单粒度（见 §6.4）、AI 字段识别逻辑（business_type/payer_type 见 §6.5）、成本控制（见 §6.7）、图片选型（见 §6.9）、模型迭代（见 §6.10）、开源闭源（见 §6.11）、多模型路由（见 §6.12）。
